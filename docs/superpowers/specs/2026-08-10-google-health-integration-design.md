...
- `Buscar Passos (dailyRollUp)` e `Buscar Distancia (dailyRollUp)` — duas chamadas `POST` separadas (uma por `dataType`, já que o path da Health API é `users/{user}/dataTypes/{dataType}/dataPoints:dailyRollUp`, um `dataType` por request), **em paralelo** (fan-out a partir de `Token para uso`, não encadeadas uma na outra — ver correção abaixo) → cada uma alimenta seu próprio node `Set` (`Anexar user_id (passos)` / `Anexar user_id (distancia)`) que namespacia a resposta bruta do Google (`steps_response` / `distance_response`) e carrega o `user_id` → `Merge` (`combine`, `combineByFields` em `user_id`, `joinMode: keepEverything` — full outer join) → `Montar payload diario` (Code) lê `$json.steps_response`/`$json.distance_response`/`$json.user_id` do item já mesclado → `Gravar google_health_daily` (upsert `on_conflict=user_id,metric_date`).
...
**Correção pós-revisão (mesmo dia)**: uma revisão da versão publicada encontrou 1 bug crítico e 4 importantes — corrigidos e republicados, versão ativa nova `184b824e-eca3-49d8-8576-573c83494b42`. Resumo (detalhe completo em `task-5-report.md`, seção "Fix round"):
1. **Crítico**: `Extrair conexao atualizada` só devolvia a primeira conexão renovada de N (`$input.first()` em vez de `$input.all()`) — com token de ~1h e cron de 4h, na prática só 1 usuário sincronizava por rodada. Corrigido para processar todos os itens com `pairedItem` explícito.
2. `Buscar Passos`/`Buscar Distancia` estavam encadeadas em série (não paralelas) e uma falha em Passos matava Distância também — corrigido com fan-out + `Merge` (full outer join por `user_id`), conforme descrito acima.
3. `metric_date` era calculado em UTC enquanto o range de busca usa hora local da instância (Brasília) — divergem entre 21h–meia-noite BRT. Corrigido para `$now.toFormat('yyyy-MM-dd')` (mesmo relógio Luxon usado nos outros nodes).
4. `expires_at` quebrava (`RangeError`) se `expires_in` viesse ausente da resposta de refresh do Google — adicionado fallback `($json.expires_in || 3600)`.
